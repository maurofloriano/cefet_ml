...
class PreprocessingMixin:
    ...
    def convert_sentiment_to_scalar_neutral_is_positive(self, df):
        def sentiment_to_scalar(data):
            if data == "negative":
                return SentimentEnum.NEGATIVE
            if data == "positive" or data == "neutral":
                return SentimentEnum.NEUTRO
            return SentimentEnum.UNKNOWN
    ...
    def preprocessing(self, df, has_neutral):
        ...
        return (
            self.convert_sentiment_to_scalar(cleaned_words_df)
            if has_neutral
            else self.convert_sentiment_to_scalar_neutral_is_positive(cleaned_words_df)
        )
